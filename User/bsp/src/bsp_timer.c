...
*	修改记录 :
*		版本号  日期        作者     说明
*		V1.0    2013-02-01 pigpeng  正式发布
*		V1.1    2013-06-21 pigpeng  增加us级延迟函数 bsp_DelayUS
*		V1.2    2014-09-07 pigpeng  增加TIM4 硬件定时中断，实现us级别定时.20us - 16秒
*		V1.3    2015-04-06 pigpeng  增加 bsp_CheckRunTime(int32_t _LastTime) 用来计算时间差值
*		V1.4	2015-05-22 pigpeng  完善 bsp_InitHardTimer() ，增加条件编译选择TIM2-5
*
*	Copyright (C),
*
*********************************************************************************************************
...
#endif
{
    ...
    if (TIM_GetITStatus(TIM_HARD, TIM_IT_CC3))
    {
        ...
        /* 先关闭中断，再执行回调函数。因为回调函数可能需要重启定时器 */
        s_TIM_CallBack3();
        // TIM_ITConfig(TIM_HARD, TIM_IT_CC3, ENABLE);	/* 禁能CC4中断 */
    }
    ...
    if (TIM_GetITStatus(TIM_HARD, TIM_IT_CC4))
    {
        ...
        /* 先关闭中断，再执行回调函数。因为回调函数可能需要重启定时器 */
        s_TIM_CallBack4();
        TIM_ITConfig(TIM_HARD, TIM_IT_CC4, ENABLE);	/* 禁能CC4中断 */
    }
}
